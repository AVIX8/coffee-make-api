...
describe('JWT', () => {
    ...
    it('User gets 403 on invalid credentials', async () => {
        await agent.post('/api/auth/login').send(incorrectUserData).expect(403)
    })
    ...
    it('User can use refresh token only once', async () => {
        ...
        await agent.post('/api/auth/refresh').send({ refreshToken }).expect(200)
        await agent.post('/api/auth/refresh').send({ refreshToken }).expect(404)
    })
    ...
    it('Refresh tokens become invalid on logout', async () => {
        ...
        await agent.post('/api/auth/logout').send({ refreshToken }).expect(200)
        await agent.post('/api/auth/refresh').send({ refreshToken }).expect(404)
    })
